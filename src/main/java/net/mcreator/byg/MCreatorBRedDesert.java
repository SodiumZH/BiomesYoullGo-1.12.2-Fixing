 ...
 public class MCreatorBRedDesert
   extends Elementsbyg.ModElement {
   @ObjectHolder("byg:breddesert")
   public static final BiomeGenCustom biome = null;
   
   public MCreatorBRedDesert(Elementsbyg instance) {
     super(instance, 214);
   }
   ...
   static class BiomeGenCustom extends Biome {
     public BiomeGenCustom() {
       super((new Biome.BiomeProperties("Red Desert")).setRainfall(0.1F).setBaseHeight(0.15F).setWaterColor(-2893433).setHeightVariation(0.1F)
           .setTemperature(2.0F));
       setRegistryName("breddesert");
     ...
     public WorldGenAbstractTree getRandomTreeFeature(Random rand) {
       return new MCreatorBRedDesert.CustomTree();
     }
   }
   ...
   static class CustomTree extends WorldGenAbstractTree {
     ...
     public boolean generate(World world, Random rand, BlockPos position) {
       ...
       if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
         ...
         if (position.getY() < world.getHeight() - height - 1) {
           world.setBlockState(position.down(), Blocks.SAND.getStateFromMeta(1), 2); int genh;
     ...
     protected boolean canGrowInto(Block blockType) {
       return (blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == Blocks.AIR
         .getDefaultState().getBlock() || blockType == Blocks.SAND.getStateFromMeta(1).getBlock() || blockType == Blocks.SAND
         .getStateFromMeta(1).getBlock());
     }
 
     
     protected void setDirtAt(World world, BlockPos pos) {
       if (world.getBlockState(pos).getBlock() != Blocks.SAND.getStateFromMeta(1).getBlock()) {
         setBlockAndNotifyAdequately(world, pos, Blocks.SAND.getStateFromMeta(1));
       }
     }
...
/* Location:              H:\eclipse-workspace-19\BiomesYouGo1.7.1-deobf.jar!\net\mcreator\byg\MCreatorBRedDesert.class
